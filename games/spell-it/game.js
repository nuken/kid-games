(function() {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        GameBridge.setupGame({
            instructions: "Look at the picture and spell the word!",
            speakInstruction: "Spell the word you see.",
            levels: [
                { id: 1, label: "Cadet (Easy)" },
                { id: 2, label: "Commander (Tricky)" }
            ],
            onStart: (level) => {
                currentLevel = level;
                startTime = Date.now();
    ...
    function handleLetterClick(char, btn) {
        ...
        if (char === currentWordLetters[targetIndex]) {
            ...
            if (remaining === 0) {
                ...
                GameBridge.celebrate("Correct! " + currentWordLetters.join(''));
