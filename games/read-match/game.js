(function() {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        GameBridge.setupGame({
            instructions: "Read the sentences. Pick the one that matches the picture.",
            levels: [
                { id: 1, label: "Cadet (2 Choices)" },
                { id: 2, label: "Commander (4 Choices)" }
            ],
            onStart: (level) => {
                currentLevel = level;
                startTime = Date.now();
        ...
        choices.sort(() => Math.random() - 0.5).forEach(txt => {
            ...
            btn.onclick = () => {
                if (txt === item.correct) {
                    ...
                    GameBridge.celebrate("That is right!");
