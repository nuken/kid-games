...
document.addEventListener('DOMContentLoaded', () => {
    ...
    GameBridge.setupGame({
        instructions: "Sort numbers! Odd goes left, Even goes right.",
        speakInstruction: "Sort the numbers into the correct bins.",
        levels: [
            { id: 1, label: "Cadet (Visual Hints)" },
            { id: 2, label: "Commander (Mental Math)" }
        ],
        onStart: (level) => {
            difficulty = level;
            score = 0;
...
function spawnNumber(silent) {
    ...
    // UPDATED: Start at 110px to match new CSS position (below legend)
    box.style.top = '110px'; 
...
function checkAnswer(type) {
    ...
    if (isCorrect) {
        ...
        GameBridge.speak("Oops. Try again.");
    }
}

function explainRules() {
    GameBridge.speak("Even numbers have buddies. Odd numbers have one lonely dot.");
}