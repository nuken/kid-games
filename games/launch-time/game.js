...
document.addEventListener('DOMContentLoaded', () => {
    ...
    GameBridge.setupGame({
        instructions: "Commander! Match the analog clock to the digital target time.",
        speakInstruction: "Set the clock to match the target time.",
        levels: [
            { id: 1, label: "Cadet (5 Mins)" },
            { id: 2, label: "Commander (1 Min)" }
        ],
        onStart: (level) => {
            difficulty = level;
...
function spawnTime(silent) {
    ...
    if (!silent) {
        ...
        GameBridge.speak("Set clock to " + timeText);
    }
}
...
function checkTime() {
    if (currentHour === targetHour && currentMin === targetMin) {
        ...
        document.getElementById('message').innerText = "Check your instruments!";
        GameBridge.speak("Not quite.");
    }
}